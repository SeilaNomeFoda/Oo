...
function topos(p546)
    ...
    -- checagens iniciais: p546 e player/character/hrp válidos
    if not p546 then
        return
    end
    if not vu547 or not vu547.Character then
        return
    end
    local hrp = vu547.Character:FindFirstChild("HumanoidRootPart")
    if not hrp or not vu547.Character:FindFirstChild("Humanoid") then
        return
    end
    if vu547.Character.Humanoid.Health <= 0 then
        return
    end

    -- agora é seguro acessar p546.Position
    local v548 = (p546.Position - hrp.Position).Magnitude
    ...
    -- garante que WaitHRP devolve algo válido antes de usar .CFrame
    local currentHRP = WaitHRP(vu547)
    if not currentHRP then
        -- fallback: usa hrp que já verificamos
        currentHRP = hrp
    end

    if not vu547.Character:FindFirstChild("PartTele") then
        local v550 = Instance.new("Part")
        v550.Size = Vector3.new(10, 1, 10)
        v550.Name = "PartTele"
        v550.Anchored = true
        v550.Transparency = 1
        v550.CanCollide = true
        v550.Parent = vu547.Character
        v550.CFrame = currentHRP.CFrame

        local vu551 = v550

        -- usar a sintaxe de método correta
        vu551:GetPropertyChangedSignal("CFrame"):Connect(function()
            -- upvalues: (ref) vu529, (ref) vu547, (ref) vu551
            if vu529 then
                task.wait()
                if vu547.Character and vu547.Character:FindFirstChild("HumanoidRootPart") then
                    local safeHRP = WaitHRP(vu547) or vu547.Character:FindFirstChild("HumanoidRootPart")
                    if safeHRP then
                        safeHRP.CFrame = vu551.CFrame
                    end
                end
            end
        end)
    ...
    -- segurança: pega a referência atual do PartTele
    local partTele = vu547.Character and vu547.Character:FindFirstChild("PartTele")
    if not partTele then
        vu529 = false
        return
    end

    local v553 = TweenService:Create(partTele, TweenInfo.new(v548 / 360, Enum.EasingStyle.Linear), {
        CFrame = p546
    })
    v553:Play()
    v553.Completed:Connect(function(p554)
        ...
        if p554 == Enum.PlaybackState.Completed then
            if vu547.Character and vu547.Character:FindFirstChild("PartTele") then
...
function TelePPlayer(p573)
    ...
    -- garante que p573 é CFrame válido
    if typeof(p573) ~= "CFrame" then
        warn("TelePPlayer recebeu valor inválido:", p573)
        return
    end
...
local function AutoFruitsSystem()
    -- Variáveis de controle (todas ativas por padrão)
    local RandomAuto = true
    local AutoStoreFruit = true
    local TweenFruit = true
    local Grabfruit = true
    
    -- 1. Auto Random Fruits
    spawn(function()
        pcall(function()
            while wait() do
                if RandomAuto then
            ...
            if AutoStoreFruit then
                pcall(function()
                    ...
                    local fruitList = {
                        ...
                        { "Tiger Fruit", "Tiger-Tiger" },  -- Alterado aqui
                    ...
                    for _, fruitData in ipairs(fruitList) do
                        ...
                        local fruitID = fruitData[2]
                        local fruit = backpack:FindFirstChild(fruitName) or character:FindFirstChild(fruitName)
                        
                        if fruit then
                            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StoreFruit", fruitID, fruit)
                            break
                        end
                    end
                end)
    ...
    -- 3. Teleport To Fruit Spawn (Fruit Collector)
    spawn(function()
        while wait(0.1) do
            if TweenFruit then
    ...
    -- 4. Auto Teleport Fruits (Instant TP)
    spawn(function()
        while wait(0.1) do
            if Grabfruit then
                for _, item in pairs(game.Workspace:GetChildren()) do
...
-- Para usar a função (todas as funcionalidades ativas por padrão):
AutoFruitsSystem()
